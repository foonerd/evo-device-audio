...
//!   `delivery.active_endpoint` verb and (in follow-on chunks)
...
//!   asound.conf pipeline on every rewire (follow-on chunk).
//!
...
/// (asound.conf re-renderer when wired in a follow-on chunk)
/// subscribe to.
