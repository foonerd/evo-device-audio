...
//! `org.evoframework.delivery.alsa` plugin.
//!
...
use org_evoframework_delivery_alsa::{AlsaDeliveryPlugin, PLUGIN_NAME};
...
fn main() -> ! {
    ...
    tracing::info!(
        ...
        plugin = PLUGIN_NAME,
        "delivery-alsa-wire starting"
    );
    let plugin = AlsaDeliveryPlugin::new();
    let config = HostConfig::new(PLUGIN_NAME);
    run_oop_and_exit(plugin, config, &socket_path, "delivery-alsa-wire")
}
...
fn parse_args() -> Result<PathBuf> {
    ...
    let path = args
        .next()
        .ok_or_else(|| anyhow!("usage: delivery-alsa-wire <socket-path>"))?;
    if args.next().is_some() {
        return Err(anyhow!(
            "usage: delivery-alsa-wire <socket-path> (too many arguments)"
        ));
    }
