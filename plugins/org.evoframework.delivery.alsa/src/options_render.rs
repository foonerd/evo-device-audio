...
/// Render target for the operator-options drop-in.
///
/// `Main` rewires the canonical producer PCM (`pcm.evo`) used by
/// receiver / standalone devices. `SourceLocal` rewires only the
/// source-host local-render PCM (`pcm.evo_local`) while leaving the
/// source producer path (`pcm.evo` -> loopback) untouched.
#[derive(Debug, Clone, Copy, PartialEq, Eq)]
pub enum RenderTarget {
    /// Render the standard `pcm.evo` override.
    Main,
    /// Render source-host local renderer (`pcm.evo_local`) override.
    SourceLocal,
}
...
pub fn render_drop_in(settings: &OptionsSettings) -> String {
    render_drop_in_for_target(settings, RenderTarget::Main)
}

/// Render the asound.conf drop-in body targeting either the standard
/// producer PCM (`pcm.evo`) or the source-host local renderer PCM
/// (`pcm.evo_local`).
pub fn render_drop_in_for_target(
    ...
    let front_pcm_name = match target {
        RenderTarget::Main => "evo",
        RenderTarget::SourceLocal => "evo_local",
    };
    let node_prefix = match target {
        RenderTarget::Main => "evo",
        RenderTarget::SourceLocal => "evo_local",
    };
    ...
    let terminus_name = format!("{node_prefix}_terminus");
    let rate_name = format!("{node_prefix}_rate");
    let mixer_name = format!("{node_prefix}_mixer");

    writeln!(out, "pcm.{terminus_name} {{").unwrap();
    ...
    if let Some(rate) = resampling_node {
        writeln!(out, "pcm.{rate_name} {{").unwrap();
    ...
    if matches!(mixer_kind, EffectiveMixer::Software) {
        writeln!(out, "pcm.{mixer_name} {{").unwrap();
    ...
    if exclusive_mode {
        writeln!(out, "pcm.{front_pcm_name} {{").unwrap();
        writeln!(out, "    type hw").unwrap();
        ...
        let slave_name = match (mixer_kind, resampling_node.is_some()) {
            (EffectiveMixer::Software, _) => mixer_name.as_str(),
            (_, true) => rate_name.as_str(),
            _ => terminus_name.as_str(),
        };
        writeln!(out, "pcm.{front_pcm_name} {{").unwrap();
        writeln!(out, "    type plug").unwrap();
...
mod tests {
    ...
    #[test]
    fn source_local_target_renders_evo_local_frontend_not_evo() {
        let s = OptionsSettings {
            mixer_type: Some(MixerType::Software),
            output_device: Some("hw:CARD=DAC,DEV=0".to_string()),
            ..Default::default()
        };
        let out = render_drop_in_for_target(&s, RenderTarget::SourceLocal);
        assert!(out.contains("pcm.evo_local {"));
        assert!(!out.contains("pcm.evo {\n"));
        assert!(out.contains("pcm.evo_local_mixer {"));
        assert!(out.contains("pcm.evo_local_terminus {"));
    }

    #[test]
    fn source_local_target_bit_perfect_keeps_evo_local_direct_hw() {
        let s = OptionsSettings {
            exclusive_mode: Some(true),
            output_device: Some("hw:CARD=USBDAC,DEV=0".to_string()),
            ..Default::default()
        ...
        let block = out
            .split("pcm.evo_local {")
            .nth(1)
            .expect("evo_local block present");
        let end = block.find('}').expect("evo_local block end");
        let body = &block[..end];
        assert!(body.contains("type hw"));
        assert!(!body.contains("type plug"));
    }
