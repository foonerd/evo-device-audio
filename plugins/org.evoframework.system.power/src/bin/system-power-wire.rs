...
//! `org.evoframework.system.power` plugin.
//!
...
use org_evoframework_system_power::{SystemPowerPlugin, PLUGIN_NAME};
...
fn main() -> ! {
    ...
    tracing::info!(
        ...
        plugin = PLUGIN_NAME,
        "system-power-wire starting"
    );
    let plugin = SystemPowerPlugin::new();
    let config = HostConfig::new(PLUGIN_NAME);
    run_oop_and_exit(plugin, config, &socket_path, "system-power-wire")
}
...
fn parse_args() -> Result<PathBuf> {
    ...
    let path = args
        .next()
        .ok_or_else(|| anyhow!("usage: system-power-wire <socket-path>"))?;
    if args.next().is_some() {
        return Err(anyhow!(
            "usage: system-power-wire <socket-path> (too many arguments)"
        ));
    }
