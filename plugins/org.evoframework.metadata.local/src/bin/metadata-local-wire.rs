...
//! `org.evoframework.metadata.local` plugin.
//!
...
//!
//! ```text
//! metadata-local-wire <socket-path>
//! ```
//!
...
fn parse_args() -> Result<PathBuf> {
    ...
    let path = args
        .next()
        .ok_or_else(|| anyhow!("usage: metadata-local-wire <socket-path>"))?;
    if args.next().is_some() {
        return Err(anyhow!(
            "usage: metadata-local-wire <socket-path> (too many arguments)"
        ));
    }
