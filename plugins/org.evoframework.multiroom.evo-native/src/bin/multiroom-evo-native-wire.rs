//! # multiroom-evo-native-wire
//!
//! Out-of-process reference binary for the
//! `org.evoframework.multiroom.evo-native` plugin.
//!
...
async fn main() -> Result<()> {
    ...
    tracing::info!(
        ...
        plugin = PLUGIN_NAME,
        "multiroom-evo-native-wire starting"
    );
...
fn parse_args() -> Result<PathBuf> {
    ...
    let path = args.next().ok_or_else(|| {
        anyhow!("usage: multiroom-evo-native-wire <socket-path>")
    })?;
    if args.next().is_some() {
        return Err(anyhow!(
            "usage: multiroom-evo-native-wire <socket-path> (too many arguments)"
        ));
    }
