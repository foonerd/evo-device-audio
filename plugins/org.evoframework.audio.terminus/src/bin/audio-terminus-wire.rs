...
//! `org.evoframework.audio.terminus` plugin.
//!
...
fn parse_args() -> Result<PathBuf> {
    ...
    let path = args
        .next()
        .ok_or_else(|| anyhow!("usage: audio-terminus-wire <socket-path>"))?;
    if args.next().is_some() {
        return Err(anyhow!(
            "usage: audio-terminus-wire <socket-path> (too many arguments)"
        ));
    }
