...
| `registry_count_diverges` | The source registry's persisted `track_count` for the floor source disagrees with MPD's `stats.songs` wire truth. Typical trigger: a full music-plane wipe leaves stale thousands in `sources.toml` because a prior rehydrate did not persist the wire-truth apply. | Floor source (`local-internal`) `SourceRecord.track_count` ≠ `mpc stats` `songs`. | `apply_track_counts_with_scan_time` + `registry.persist()` + `library::publish_subjects` to refresh the `audio_library_state` envelope in the same sweep. | yes |
