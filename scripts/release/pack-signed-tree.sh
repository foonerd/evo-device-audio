...
#
# Writes into --out-dir:
#   tree.tar.gz  tree.tar.gz.sig  tree.tar.gz.sha256
#   build-info.toml  build-info.sig
#
# Usage:
#   EVO_PLUGIN_SIGNING_KEY=/path/to.pem \
#   scripts/release/pack-signed-tree.sh \
#     --piece NAME --version VER --kind KIND --src DIR --out-dir DIR \
#     [--target TRIPLE]
...
# Deterministic tree: one top-level directory named after the piece.
STAGE="${TMP}/${PIECE}"
mkdir -p "${STAGE}"
cp -a "${SRC}/." "${STAGE}/"
